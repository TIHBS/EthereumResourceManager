...
contract ResourceManager {
    ...
    modifier isTxOwner(string memory txId) {
        require(
            ...
            "Message sender is not owner of global transaction!"
        );
        _;
    }
    ...
    modifier isTxStarted(string memory txId) {
        require(
            ...
            "The transaction is in invalid state!"
        );
        _;
    }
    ...
    // for testing
    function getBeforeImage(string memory variableName)
        internal
        view
    ...
    // for testing
    function getValueNoLocking(string memory variableName)
        internal
        view
        returns (string memory)
    {
    ...
    function getLockedVariables(string memory txId)
        internal
        view
        returns (string[] memory)
    {
    ...
    function begin(string calldata txId) external {
        require(
            txs[txId].owner == address(0),
            "The global transaction is already started!"
        );
        txs[txId].owner = tx.origin;
        txs[txId].state = TxState.STARTED;
    ...
    function commit(string calldata txId)
        external
        isTxOwner(txId)
        isTxStarted(txId)
    {
    ...
    function abort(string calldata txId)
        external
        isTxOwner(txId)
        isTxStarted(txId)
    {
        ...
        for (uint256 i = 0; i < lockedVariables.length; i++) {
            if (isWL(lockedVariables[i])) {
                string memory beforeImage = variables[lockedVariables[i]]
                    .beforeImage;
    ...
    function setValue(
        ...
        string memory txId,
        string memory value
    ) internal isTxOwner(txId) isTxStarted(txId) {
        bool acquiredLock;
        bool hasSetLock;
        (acquiredLock, hasSetLock) = acquireLock(
            variableName,
            txId,
            LockType.WRITE_LOCK
        );
    ...
    function getValue(string memory variableName, string memory txId)
        internal
        isTxOwner(txId)
        isTxStarted(txId)
        returns (string memory)
    ...
    function canObtainLock(
        ...
        if (
            ...
            !(variable.readLocksCount == 1 && variable.hasReadLock[txId])
        ) {
            return false;
        }
    ...
    function lock(
        ...
        if (lockType == LockType.READ_LOCK) {
            ...
            if (
                !variable.hasReadLock[txId] &&
                !compareStrings(variable.writeLock, txId)
            ) {
    ...
    function releaseLock(string memory variableName, string memory txId)
        private
    {
        ...
        if (isWL(variableName)) {
            ...
            // Otherwise, we certainly have a readlock on the variable (we know we have some lock :)).
            releaseRL(variableName, txId);
        }
    }
    ...
    function copyString(string memory value)
        internal
        pure
        returns (string memory)
    {
